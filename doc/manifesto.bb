@(lv 1)
@@title Markup with More Structures
...
@(lv 2)
@@title Yet another markup language, why?
...
@(lv 3)
@@title What makes a good markup language?
...
@(style number 1)
@list
@@item Minimal effort to typeset, potentially with the help of an IDE
@@item Visual similarity to how it should be rendered to the reader
@@item Semantic clarity of structural content only by looking at the syntax
@@item Programmability for more functionality and rich interaction
@end
...
(I don't want to edit the table by hand, so I'm leaving emojis as is here.)
...
#emoji(no_entry_sign) - not applicable

#emoji(warning) - concerning

#emoji(thinking) - questionable

#emoji(thinking)#emoji(ballot_box_with_check) - reasonable after all

#emoji(ballot_box_with_check) - fairly good
...
@(lv 2)
@@title What's your idea then?

Our goal can actually be broken down into two steps:

@(style number 1)
@list
@@item Make a text-based structural language by carefully design its syntax, and then
@@item Provide programmability through semantic actions.
@end
...
@(lv 3)
@@title Surface Syntax and Core Syntax
...
An atom, similar to atoms in lisp languages, are basic identifiers. The actual design decision is rather flexible. Personally, I don't refuse emojis or string literals, but not white spaces or the escape character @code{\\}. We denote atoms as @code{atom} in the paragraphs to come. Apart from atoms, whitespaces need to be distinguished during lexical analysis as well.

Blocks are how the language organizes structures. Call it "list" if you're a lisp fan. The symbol @code{\@}, followed immediately by an atom, is a block delimiter. A pair of @code{\@atom} and @code{\@end} forms a block that can contain or live in other blocks, along with other arbitrary text contents. The core syntax may be designed to wrap all uncontained text segments into (@code{\@text}, @code{\@end}) blocks. The whole document is implicitly wrapped by a user-specified block in core syntax, such as (@code{\@text}, @code{\@end}) or (@code{\@md}, @code{\@end}).

A pair of (@code{\@block}, @code{\@end}) delimiters does nothing other than structural organization, so it's called a parentheses block.

Some blocks that can fit into one line can also be written in the following sequence: @code{\@atom}, white spaces, @code{\{}, inner content, and @code{\}}. The curly brace syntax can be desugared into normal block delimiters in core syntax. The formatter is suggested to change curly braces back to @code{\@end} on a line warp.

If some "consecutive" content can be included in a block, one may choose to write @code{\@\@atom}, delimited by a single line break @code{\\n} instead of @code{\@end} to denote a block, which will be desugared in an obvious way in core syntax. This is called an in-context block, meaning that the content is somewhat inlined. An example use case is a list item in markdown. Since a list should not contain paragraphs that divides the list (which makes the whole list visually confusing),

Blob blocks parses inner content separately. Delimited by @code{\@\@\@} and @code{\@\@\@}, any content inside will be handled as is. If @code{\@\@\@} is included in the content, just add more @code{\@}s to the delimiters.

Enough for the blocks. Time for annotations and items.

The syntax for annotation @code{\@(lisp)} is a symbol @code{\@} immediately followed by a well-delimited @code{(} @code{)} s-expression, with atoms as terminals inside. The @code{(lisp)} above simply represents an s-expression. Semantically, annotations are attached to the block immediately after itself. A block may have any number of annotations, forming a list whose order is determined by the order of appearance of annotations for this block in the document.

An in-context annotation @code{\@\@(lisp)} is an annotation attached to the surrounding block. The motivation is to allow more flexible annotation that have more comments in context. It can be trivially desugared.

(Do we really need this? It introduces an ambiguity.) A block may contain also annotations itself as in @code{\@atom(lisp)} and @code{\@end(lisp)}, desugaring to @code{\@(lisp)\@atom} and @code{\@\@(lisp)\@end}, respectively.

Apart from using @code{(} and @code{)} as s-expression delimiters, @code{[} and @code{]} should also be considered. In this language, @code{[} and @code{]} are used for collections of homogeneous data (not quite), while @code{(} and @code{)} are for heterogeneous ones. A typical example is representing an array/tuple @code{[1 0]} v.s. a map/anonymous record @code{[(x 1) (y 0)]} v.s. a named record @code{(vec (x 1) (y 0))} v.s. a named tuple @code{(vec 1 0)}. The motivation is that when using vanilla s-expressions to represent metadata, I found some reasonable critics about their appearance and visual attractiveness. Personally, I think the most important issue among all is not being able to tell the intension of the author in a glance, mostly due to the language's over-simplicity. However, the actual semantics of @code{[} and @code{]} should be further reconciled since the underneath rationale is not satisfying so far. Maybe @code{(name ...)} should always contain a @code{name} at the beginning to give the whole expression a theme, while @code{[...]} means it's all about data. Anyways, from now on, we denote an s-expression involving both delimiters as @code{(lisp)}. And a series of @code{\@(lisp)\@(lisp)...} is equivalent to @code{\@[(lisp) (lisp)]...}.

Not all structural content should have children. Those that shouldn't can be represented as items @code{\#atom(lisp)} (or @code{\#atom}, short for @code{\#atom()}). Items can be used to represent links, references (backlinks), or even inclusion of other documents. To encode how the link is rendered (e.g. as url text, as an image, as a pdf, or even as a webpage), the lisp-formatted attributes provides extra expressivity. Items can also be annotated. Blocks and items are collectively called entities.

Finally, the escapes. The only necessary escapes throughout the document are @code{\\\\}, @code{\\\@}, @code{\\\#}, @code{\\\{}, and @code{\\\}}.
...
As mentioned, links are accompanied with how it's supposed to be rendered. An image is encoded as @code{\#link(image url)}, where a @code{url} is a string literal or raw string literal and if it's a file path, it can be either absolute or relative. Besides @code{image}, @code{pdf} and @code{web} are other available options. Text links, despite most frequently used, are not @code{\#link} items. Instead, they should be treated as normal blocks containing text (or actually anything), but annotated with @code{\@(link url)}. With the link annotation, the whole block will be "clickable" when being hovered upon, and by clicking the user jumps to the url.
...
The content inside a block, or the inner content of a block, is suggested to be indented, and the indentation will be automatically removed following the method shown in the @(link "https://docs.rs/indoc/latest/indoc/")@text{indoc} crate. Inner contents are trimmed by default. If needed, whitespaces can be manually added with @code{\#space} items.

@(lv 4)
@@title Bindings and Namespaces

@code{\@(ref name)} gives the annotated entity (either block or item) a @code{name} that can be referred to elsewhere. @code{\#link(ref name)} is such a reference. A block may have multiple names and multiple references at the same time. The scope of these names remains to be further designed. Roughly speaking, the scope could be at document level.
