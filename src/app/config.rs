...
/// Uses `[tasks.amplify]`, `[tasks.distill]`, `[tasks.atomize]`, `[tasks.probe]` TOML tables.
///
/// [`TaskKind`]: crate::llm::TaskKind
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct TaskSettings {
    /// Settings for the amplify task.
    #[serde(default = "TaskConfig::default_amplify")]
    pub amplify: TaskConfig,
    /// Settings for the distill task.
    #[serde(default = "TaskConfig::default_distill")]
    pub distill: TaskConfig,
    /// Settings for the atomize task.
    #[serde(default = "TaskConfig::default_atomize")]
    pub atomize: TaskConfig,
    /// Settings for the probe task.
    #[serde(default = "TaskConfig::default_probe")]
    pub probe: TaskConfig,
}

impl Default for TaskSettings {
    fn default() -> Self {
        Self {
            amplify: TaskConfig::default_amplify(),
...
pub struct TaskConfig {
    ...
    #[serde(rename = "token-limit", default = "default_distill_tokens")]
...
impl TaskConfig {
    /// Default config for the distill task.
    pub fn default_distill() -> Self {
        Self {
            provider: default_provider_name(),
            model: default_model_name(),
            token_limit: default_distill_tokens(),
    ...
    /// Default config for the amplify task.
    pub fn default_amplify() -> Self {
        Self {
            provider: default_provider_name(),
            model: default_model_name(),
            token_limit: default_amplify_tokens(),
            system_prompt: String::new(),
            user_prompt: String::new(),
        }
    }

    /// Default config for the probe task.
    pub fn default_probe() -> Self {
        Self {
            provider: default_provider_name(),
            model: default_model_name(),
            token_limit: default_probe_tokens(),
            system_prompt: String::new(),
            user_prompt: String::new(),
        }
    }
...
fn default_distill_tokens() -> MaxTokens {
    MaxTokens::UNLIMITED
}
fn default_atomize_tokens() -> MaxTokens {
    MaxTokens::UNLIMITED
}
fn default_amplify_tokens() -> MaxTokens {
    MaxTokens::UNLIMITED
}
fn default_probe_tokens() -> MaxTokens {
    MaxTokens::UNLIMITED
}
...
mod tests {
    ...
    fn task_settings_defaults_are_sensible() {
        ...
        assert!(tasks.distill.token_limit.is_unlimited());
        assert!(tasks.atomize.token_limit.is_unlimited());
        assert!(tasks.amplify.token_limit.is_unlimited());
        assert!(tasks.probe.token_limit.is_unlimited());
        assert_eq!(tasks.distill.provider, "openai");
        assert_eq!(tasks.distill.model, "gpt-5.2");
    }
    ...
    fn task_settings_round_trips_through_toml() {
        ...
        assert_eq!(parsed.tasks.distill.provider, "deepseek");
        assert_eq!(parsed.tasks.distill.model, "deepseek-chat");
        assert_eq!(parsed.tasks.distill.token_limit.raw(), 300);
        assert_eq!(parsed.tasks.atomize.token_limit.raw(), 0);
        assert_eq!(parsed.tasks.amplify.token_limit.raw(), 0);
        assert_eq!(parsed.tasks.probe.token_limit.raw(), 0);
    }
    ...
    fn task_settings_missing_from_toml_uses_defaults() {
        ...
        assert!(config.tasks.distill.token_limit.is_unlimited());
        assert!(config.tasks.atomize.token_limit.is_unlimited());
        assert!(config.tasks.amplify.token_limit.is_unlimited());
        assert!(config.tasks.probe.token_limit.is_unlimited());
        assert_eq!(config.tasks.distill.provider, "openai");
    }
}
