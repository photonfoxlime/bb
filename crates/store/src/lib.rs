...
//!
//! This crate owns the persisted document model and the file-backed load/save
//! surface for `blocks.json` plus mounted subtrees.
//!
//! Application-specific navigation, search, and LLM-context helpers stay in
//! the `blooming-blockery` crate because they depend on app-local `llm` and
//! `text` modules. The application crate reintroduces those helpers as an
//! extension trait over [`BlockStore`].
//!
//! # Module layout
//!
//! Start with this file (`lib.rs`) for [`BlockStore`] and its fundamental
...
//!    duplicate, remove) and tree-traversal helpers.
...
//! 6. [`paths`] -- store-specific filesystem locations (`blocks.json` and its parent).
