...
1. When `ReduceMessage::Start` or `ExpandMessage::Start` runs, the app computes a **request signature** from the full lineage (root-to-target points) used to build the LLM prompt.
2. The signature is stored in `pending_reduce_signatures` or `pending_expand_signatures` for that block.
3. When `ReduceMessage::Done` or `ExpandMessage::Done` arrives, the app recomputes the current lineage signature for the same block.
...
- Cancel actions remove pending signatures, so late responses after cancel are also ignored.
